  ...
  persist(
    (set, get) => ({
      ...
      addBook: async (book) => {
        try {
          const id = createLocalId();
          await api.createBook({ ...book, id });
          set((state) => ({ books: [...state.books, { ...book, id }] }));
        } catch (error) {
      ...
      addSuggestion: async (suggestion) => {
        try {
          const id = createLocalId();
          await api.createSuggestion({ ...suggestion, id });
          set((state) => ({ suggestions: [...state.suggestions, { ...suggestion, id }] }));
        } catch (error) {
      ...
      addPoll: async (poll) => {
        try {
          ...
          set((state) => ({
            polls: [...state.polls, { id, question: poll.question, options, totalVotes: 0 }],
          }));
        } catch (error) {
      ...
      addInterview: async (interview) => {
        try {
          ...
          await api.createInterview({ ...interview, id, dialogues });
          set((state) => ({ interviews: [...state.interviews, { ...interview, id, dialogues }] }));
        } catch (error) {
