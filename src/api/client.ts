...
export async function updatePoll(id: string, data: any) {
  if (data.question !== undefined) {
    await must(
      supabase.from("polls").update({ question: data.question }).eq("id", id),
      "Anket sorusu güncellenemedi"
    );
  }
...
export async function fetchInterviews() {
  ...
  const dialogues = await must(
    supabase.from("dialogues").select("id, interview_id, speaker, text"),
...
export async function createInterview(interview: any) {
  ...
  const dialogues = (interview.dialogues ?? []).map((dialogue: any) => ({
...
export async function updateInterview(id: string, data: any) {
  ...
  if (Array.isArray(data.dialogues)) {
    ...
    const dialogues = data.dialogues.map((dialogue: any) => ({
      id: dialogue.id ?? generateId(),
      interview_id: id,
      speaker: dialogue.speaker,
      text: dialogue.text,
